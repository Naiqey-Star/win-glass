...
从 [**Releases 页面**](../../releases/latest) 下载 `win_glass_setup_v1.6.0.exe`，双击即可。
...
1. 双击 `win_glass_setup_v1.6.0.exe` 完成安装
...
```bat
win_glass_setup_v1.6.0.exe --autostart
```

取消开机自启：`win_glass_setup_v1.6.0.exe --no-autostart`
...
| **右键** | 弹出菜单：暂停、立即恢复、悬停开关、全屏开关、**四个滑块**、**渐隐时间**、打开日志、退出 |
...
| `dist\win_glass_setup_v1.6.0.exe` | 安装包（内含上面两个 + 图标 + 卸载器） |
